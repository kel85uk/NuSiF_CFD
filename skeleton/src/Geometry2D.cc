  ...
  for(i=1;i<=imax;i++)
     for(j=1;j<=jmax;j++){
        if (!(geometry_mesh_[i + length_x*j] & C_F))
					nBlocks++;
					geometry_mesh_[i + length_x*j] += ((geometry_mesh_[i-1 + length_x*j] & C_F)*B_W + (geometry_mesh_[i+1 + length_x*j] & C_F)*B_E + (geometry_mesh_[i + length_x*(j-1)] & C_F)*B_S + (geometry_mesh_[i + length_x*(j+1)] & C_F)*B_N)/C_F;
